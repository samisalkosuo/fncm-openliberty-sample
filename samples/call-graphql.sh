#!/bin/bash
# call-graphql.sh — Call the CP4BA Content Services GraphQL API.
...
# Step 3 — GraphQL call
xsrf_token="a251fb4a-88df-4d9d-b38f-5ce80e603e22"
xsrf_token_header="ECM-CS-XSRF-Token:${xsrf_token}"
xsrf_token_cookie="Cookie:ECM-CS-XSRF-Token=${xsrf_token}"

curl -X POST -k -v \
  "${CP4BA_HOST}/content-services-graphql/graphql" \
  --header "${xsrf_token_header}" \
  --header "${xsrf_token_cookie}" \
